...
function Linux:PlaySound(soundId)
    spawn(function()
        ...
        if not success then
            warn("Linux UI: Failed to play sound: " .. tostring(soundId))
        end
    end)
end
...
function Linux.SaveConfig(name)
    if not name or name == "" then
        warn("Linux UI Config: Invalid configuration name provided")
    ...
    local success, result = pcall(function()
        ...
        if writefile then
            ...
            warn("Linux UI Config: writefile function not available - configs cannot be saved")
    ...
    if success and result == true then
        ...
        warn("Linux UI Config: Error saving configuration - " .. (result or "Unknown error"))
...
function Linux.LoadConfig(name)
    ...
    local success, result = pcall(function()
        if readfile and isfile(Linux.ConfigFolder .. "/" .. name .. ".json") then
            ...
            warn("Linux UI Config: Configuration file not found - " .. name .. ".json")
            return false, "Configuration file not found"
        end
    end)
    
    if not success then
        warn("Linux UI Config: Error loading configuration - " .. (result or "Unknown error"))
    end
...
function Linux.DeleteConfig(name)
    ...
    local success, result = pcall(function()
        if delfile and isfile(Linux.ConfigFolder .. "/" .. name .. ".json") then
            ...
            warn("Linux UI Config: Configuration file not found or delfile function not available")
    ...
    if success and result == true then
        ...
        warn("Linux UI Config: Error deleting configuration - " .. (result or "Unknown error"))
...
function Linux.LoadConfigList()
    ...
    local success, result = pcall(function()
        if listfiles and isfolder(Linux.ConfigFolder) then
            ...
            warn("Linux UI Config: listfiles function not available or folder not found")
            return false, "listfiles function not available or folder not found"
        end
    end)
    
    if not success then
        warn("Linux UI Config: Error loading config list - " .. (result or "Unknown error"))
    end
...
function Linux.Create(config)
    ...
    if config.Logo and config.Logo.Image then
        local logoSize = config.Logo.Size or UDim2.new(0, 20, 0, 20)
        LogoImage = Linux.Instance("ImageLabel", {
            Parent = TopBar,
            BackgroundTransparency = 1,
            Size = logoSize,
            Position = UDim2.new(0, 8, 0.5, -(logoSize.Y.Offset/2)),
            Image = config.Logo.Image,
            ImageColor3 = config.Logo.Color or Color3.fromRGB(255, 255, 255),
            ZIndex = 2
        })
        
        if config.Logo.CornerRadius then
            Linux.Instance("UICorner", {
                Parent = LogoImage,
                CornerRadius = UDim.new(0, config.Logo.CornerRadius)
            })
        end
        
        logoOffset = logoSize.X.Offset + 12
    end
    ...
    local TitleLabel = Linux.Instance("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Size = UDim2.new(0, 0, 1, 0),
        Position = UDim2.new(0, 12 + logoOffset, 0, 0),
    ...
    if config.Subtitle then
        SubtitleLabel = Linux.Instance("TextLabel", {
            ...
            BackgroundTransparency = 1,
            Size = UDim2.new(0, 0, 1, 0),
            Position = UDim2.new(0, TitleLabel.AbsoluteSize.X + 20 + logoOffset, 0, 0),
            Font = Enum.Font.GothamSemibold,
        ...
        TitleLabel:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
            SubtitleLabel.Position = UDim2.new(0, TitleLabel.AbsoluteSize.X + 20 + logoOffset, 0, 0)
        end)
    end
    ...
    function LinuxLib.Tab(config)
        ...
        function TabElements.Image(config)
            ...
            local maxSizeLimit = config.MaxSize or UDim2.new(0, 200, 0, 150)
            local imageSize = config.Size or UDim2.new(0, 150, 0, 100)
            
            if imageSize.X.Offset > maxSizeLimit.X.Offset then
                imageSize = UDim2.new(0, maxSizeLimit.X.Offset, 0, imageSize.Y.Offset)
            end
            if imageSize.Y.Offset > maxSizeLimit.Y.Offset then
                imageSize = UDim2.new(0, imageSize.X.Offset, 0, maxSizeLimit.Y.Offset)
            end
            
            local frameHeight = imageSize.Y.Offset + (hasDescription and 50 or 30)
            
            local ImageFrame = Linux.Instance("Frame", {
                Parent = Container,
            ...
            local ImageDisplay = Linux.Instance("ImageLabel", {
                Parent = ImageFrame,
                BackgroundColor3 = Color3.fromRGB(16, 16, 16),
                ...
                BorderColor3 = Color3.fromRGB(39, 39, 42),
                Size = imageSize,
                Position = UDim2.new(0.5, -(imageSize.X.Offset/2), 0, hasDescription and 45 or 25),
            ...
            local function SetImage(imageId)
                ImageDisplay.Image = imageId or ""
                if config.Callback then
                    spawn(function() Linux:SafeCallback(config.Callback, imageId) end)
                end
            end
            
            local function SetSize(newSize)
                if newSize.X.Offset > maxSizeLimit.X.Offset then
                    newSize = UDim2.new(0, maxSizeLimit.X.Offset, 0, newSize.Y.Offset)
                end
                if newSize.Y.Offset > maxSizeLimit.Y.Offset then
                    newSize = UDim2.new(0, newSize.X.Offset, 0, maxSizeLimit.Y.Offset)
                end
                
                ImageDisplay.Size = newSize
                ImageDisplay.Position = UDim2.new(0.5, -(newSize.X.Offset/2), 0, hasDescription and 45 or 25)
                
                local newFrameHeight = newSize.Y.Offset + (hasDescription and 50 or 30)
                ImageFrame.Size = UDim2.new(1, -8, 0, newFrameHeight)
            end
            
            if config.Image then
                spawn(function() Linux:SafeCallback(config.Callback, config.Image) end)
            end
            ...
            return {
                ...
                SetImage = SetImage,
                SetSize = SetSize,
                GetImage = function() return ImageDisplay.Image end
            }
        end
